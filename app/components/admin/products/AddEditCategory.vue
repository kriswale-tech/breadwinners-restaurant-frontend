...
async function handleSubmit() {
  ...
  try {
    ...
    toast.add({
      ...
      description: errorResponse.detail || `Failed to ${isEdit.value ? 'update' : 'create'} category`,
      color: 'error',
...
<template>
  <UModal :open="open" :title="isEdit ? 'Edit category' : 'Add category'"
    ...
    <template #body>
      <form id="add-edit-category-form" class="space-y-5" @submit.prevent="handleSubmit">
        ...
        <UFormField label="Description" name="description">
          <UTextarea v-model="description" placeholder="Short description of the category..." :rows="3"
            class="w-full" />
        </UFormField>
      </form>
    </template>
    <template #footer>
      <div class="flex justify-end gap-2 w-full">
        ...
        <UButton type="submit" form="add-edit-category-form" color="success" label="Save" class="ml-auto"
          :loading="submitting" :disabled="!name || submitting" />
      </div>
    </template>
  </UModal>
</template>
